const fs = require('fs');
const bytes = fs.readFileSync('./wasm_types.wasm');
...
let importObject = {
 ...
 env: {
    // Numbers in JavaScript are 64-bit floating-point variables.
...
( async () => {
    let obj = await WebAssembly.instantiate(new Uint8Array
   (bytes),
    importObject);
