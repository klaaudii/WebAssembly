;; WebAssembly module in WAT, which will manage the object movement, 
;; collision detection, and bitmap image data.
...
;; squaring the canvas size (because we chose a square canvas) and then
;; multiplying by 4 because four bytes are used for each pixel.
 (func $clear_canvas
    (local $i i32)
...
;;  dont know why firt location and then color
 i32.store ;; store color in memory location 
)

;; draw multi pixel object as a square given coordinates $x,
;; $y and color $c
(func $draw_obj
...
(loop $outer_loop (block $outer_break
 ...
 (call $draw_obj (local.get $x1) (local.get $y1) (global.get
 $no_hit_color))
 else ;; if $i_hit == 1 (hit)
 (call $draw_obj
